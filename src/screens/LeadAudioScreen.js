...
const LeadAudioScreen = ({ navigation }) => {
  ...
  const loadRecordings = useCallback(async () => {
    setLoading(true);
    try {
      const [allAudio, allLeads] = await Promise.all([
      ...
      for (const audio of allAudio) {
        if (audio.parentType !== 'lead') continue;
        const info = await FileSystem.getInfoAsync(audio.filePath).catch(() => ({ exists: false }));
        if (!info.exists) continue;
        const lead = leadMap[audio.parentId];
        combined.push({
          ...audio,
          leadName: lead?.name || 'Unknown',
          companyName: lead?.company || '',
          leadId: audio.parentId,
        });
      }
      setRecordings(combined);
    } catch (e) {
      console.warn('LeadAudio load error:', e);
    }
    setLoading(false);
  }, []);
  ...
  const deleteAudio = useCallback((item) => {
    Alert.alert(
      'Delete Recording',
      `Remove this audio from "${item.leadName}"?\nThe lead itself will NOT be deleted.`,
      [
  ...
  const downloadAudio = useCallback(async (item) => {
    try {
      ...
      const fileName = `${safeName}_audio.${ext}`;
      ...
      <View style={styles.card}>
        <TouchableOpacity
          ...
          <View style={styles.cardInfo}>
            ...
            {/* Playback controls & progress — visible when this card is active */}
            {isActive && (
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyTitle}>No Audio Recordings</Text>
            <Text style={styles.emptyText}>
              Voice notes recorded during lead creation will appear here
            </Text>
          </View>
        }
      />
...
export default LeadAudioScreen;
